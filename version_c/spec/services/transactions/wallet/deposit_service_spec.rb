describe Transactions::Wallet::DepositService do
  ...
  let!(:wallet) { create(:user_wallet, user: user) }
  ...
  context "when amount is lower or equal to 0" do
    it "raises error that indicates amount must be bigger or equal to 0" do
      expect do
        described_class.call(user: user, target_wallet_id: wallet.id, amount: -1)
      end.to raise_error(Transactions::WalletError::AmountMustBeBiggerThanZero)
    end
  end

  context "when user doesn't have any wallet" do
    it "raises error that indicates that user currently don't have any wallet" do
      expect do
        described_class.call(user: user, target_wallet_id: -1, amount: 1)
  ...
  context "when user try to deposit to other user's wallet" do
    it "raises error that indicates that user cannot do transaction to other people's wallet" do
      expect do
        described_class.call(user: user, target_wallet_id: wallet2.id, amount: 1)
