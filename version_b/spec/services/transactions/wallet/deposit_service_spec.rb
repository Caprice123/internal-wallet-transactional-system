describe Transactions::Wallet::DepositService do
  ...
  context "when logged in user try to do deposit to its own wallet from its own wallet" do
    it "raises error that indicates logged in account cannot do transaction with its own wallet" do
      ...
      expect do
        described_class.call(account: account, target_wallet_id: account.wallet.id, amount: 1)
      end.to raise_error(Transactions::WalletError::CannotTransactionWithItsOwnWallet)
    end
  end

  context "when target wallet is not found" do
    it "raises error that indicates target wallet is not valid" do
      create(:wallet, account: account, balance: 5)
